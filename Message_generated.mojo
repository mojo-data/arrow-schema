...
struct RecordBatch:
    ...
    fn buffers(self, i: Int) -> Buffer:
        var start = flatbuffers.field_vector(self._buf, int(self._pos), 8) + i * 16
    ...
    fn variadicBufferCounts(self, i: Int) -> Int64:
        return flatbuffers.read[DType.int64](self._buf, flatbuffers.field_vector(self._buf, int(self._pos), 12) + i * 8)
        ...
        *,
        ...
        if len(variadicBufferCounts) > 0:
            ...
            for o in variadicBufferCounts.__reversed__():
                builder.prepend(o[])
            _variadicBufferCounts = builder.end_vector(len(variadicBufferCounts))
...
struct Message:
    ...
    fn header_as_Schema(self) -> Schema:
        return Schema(self._buf, flatbuffers.field_table(self._buf, int(self._pos), 8).or_else(0))

    fn header_as_DictionaryBatch(self) -> DictionaryBatch:
        return DictionaryBatch(self._buf, flatbuffers.field_table(self._buf, int(self._pos), 8).or_else(0))

    fn header_as_RecordBatch(self) -> RecordBatch:
        return RecordBatch(self._buf, flatbuffers.field_table(self._buf, int(self._pos), 8).or_else(0))

    fn header_as_Tensor(self) -> Tensor:
        return Tensor(self._buf, flatbuffers.field_table(self._buf, int(self._pos), 8).or_else(0))

    fn header_as_SparseTensor(self) -> SparseTensor:
        return SparseTensor(self._buf, flatbuffers.field_table(self._buf, int(self._pos), 8).or_else(0))
