...
fn field_table(buf: DTypePointer[DType.uint8], pos: Int, field_offset: Int) -> Optional[Int32]:
    ...
    if relativ_value_offset == 0:
        return None
    return pos + relativ_value_offset + buf.offset(pos + relativ_value_offset).bitcast[DType.int32]()[0]
...
fn field_vector(buf: DTypePointer[DType.uint8], pos: Int, field_offset: Int) -> Int:
    ...
    if relativ_value_offset == 0:
        return 0
    return int(pos + relativ_value_offset + buf.offset(pos + relativ_value_offset).bitcast[DType.int32]()[0]) + 4

fn field_vector_len(buf: DTypePointer[DType.uint8], pos: Int, field_offset: Int) -> Int:
    var relativ_value_offset = _relative_field_offset(buf, pos, field_offset)
    if relativ_value_offset == 0:
        return 0
    var vec_pos = int(pos + relativ_value_offset + buf.offset(pos + relativ_value_offset).bitcast[DType.int32]()[0])
...
fn field_string(buf: DTypePointer[DType.uint8], pos: Int, field_offset: Int) -> StringRef:
    ...
    if relativ_value_offset == 0:
        return ""
    var str_pos = int(pos + relativ_value_offset + buf.offset(pos + relativ_value_offset).bitcast[DType.int32]()[0])
...
struct Builder:
    ...
    fn prep(inout self, size: Int, additional_bytes: Int):
        ...
        if self.capacity <= self.head + additional_bytes + size + align_size:
            ...
            while new_capacity <= self.head + additional_bytes + size + align_size:
                new_capacity *= 2
            var old_buf = self.buf
            self.buf = DTypePointer[DType.uint8].alloc(new_capacity)
            memcpy(self.buf.offset(new_capacity - self.head), old_buf.offset(self.capacity - self.head), self.head)
            old_buf.free()
    ...
    fn prepend(inout self, offset: Offset):
        self.prep(4, 0)
        debug_assert(offset.value <= self.head, "offset points outside of the buffer")
        self.prepend(self.head - offset.value + 4)

    fn start_vector(inout self, elem_size: Int, num_elements: Int, alignment: Int):
        self.start_nesting()
    ...
    fn end_object(inout self) -> Offset:
        ...
        if existing_vtable:
            ...
            self.buf.offset(self.start()).bitcast[DType.int32]()[0] = existing_vtable - object_offset
        else:
            self.buf.offset(self.capacity - object_offset).bitcast[DType.int32]()[0] =  self.head - object_offset
