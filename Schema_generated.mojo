...
struct Field:
    ...
    fn type_as_List(self) -> List_:
        return List_(self._buf, flatbuffers.field_table(self._buf, int(self._pos), 10).or_else(0))
    ...
    fn custom_metadata(self, i: Int) -> KeyValue:
        var start = flatbuffers.field_vector(self._buf, int(self._pos), 16) + i * 4
        start += int(flatbuffers.indirect(self._buf, start))
...
struct Schema:
    ...
    fn fields(self, i: Int) -> Field:
        var start = flatbuffers.field_vector(self._buf, int(self._pos), 6) + i * 4
        start += int(flatbuffers.indirect(self._buf, start))
        return Field(self._buf, start)
